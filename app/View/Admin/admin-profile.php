<section x-data="{open : true}" class="sm:ml-64">
    <div class="container mx-auto px-4 py-4 lg:px-20 lg:py-28 items-center justify-center mt-12">
        <?php if (isset($model['error'])) { ?>
            <div class="w-full lg:w-9/12 mx-auto mb-10">
        ...
        <?php } ?>
        <div class="p-4 mt-5">
            <h1 class="font-rubik font-medium text-4xl text-center mb-5">Ganti profil</h1>
            <div class="border-2 border-abu rounded-lg w-full lg:w-9/12  mx-auto bg-dark-white">
                <form action="/admin/profile" method="post" class="flex flex-col gap-3">
                    ...
                    <div class="flex flex-col gap-1 px-4">
                        ...
                        <input type="text" name="name" class="border-2 border-abu rounded-lg p-2" value="<?= $model['admin']['name'] ?>">
                    </div>
                    <div class="flex items-center justify-center mb-4 px-4 mt-4">
                        <button type="submit" class="font-semibold text-dark-white text-center bg-biru rounded-lg text-xl w-full p-2" onclick="alert('Anda yakin ingin mengubah nama?')">Ganti</button>
                    </div>
                </form>
            </div>
        </div>
